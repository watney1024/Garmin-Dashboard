...
.agents/skills/
  ...
  coach-<new_id>/                             # 目录名 = "coach-" + id（id 的 _ → -）
    ...
    references/
      coach-<new_id>_zh.md                    # 完整方法论正文（9 固定章节）
      coach-<new_id>_en.md
```
...
### 第 9 节"必检项"约定（所有 coach 必须有）

第 9 节是**固定四项**的逐项回答，条目内容按本流派口径写，但四项不可缺、不可换序：

1. **强度构成**：本周各强度（E/质量课）的占比口径与上限，超出时先动谁。
2. **疲劳量化与管理**：本流派如何量化疲劳（负荷/时长/配速漂移/心率等）、何时减量。
3. **大小周期**：小周期（周）形态与大周期（阶段）划分，恢复周怎么放。
4. **轻松跑口径**：E/低强度课以**心率与时间**为准，不看配速与距离（表值只作参考端）。
