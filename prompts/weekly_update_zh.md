...
5. **顺带算「力量课密度」（信息项，不判灯）**：主表里 `活动类型 = 力量训练` 的行，取
   **`移动时长 ÷ 时长`**，逐次列出并给周均值，写进复盘。读法见 docs/06 §3 末条：密度低＝组间休息
   占比高、同样时长里实际刺激更少，但**对跑步恢复的干扰也更小**（20–40%、且平均心率明显低于 E 区，
   就是"补充项"该有的样子）。⚠ **抱石不适用**（Garmin 对它不产生移动时间，恒为 0）；
   ⚠ **重量与每组次数在 Garmin 数据里根本不存在**，要判断负荷只能让我手记——**别从密度反推"该加量"**。
6. 出复盘：按 docs/06 四段式把一节追加到 <WEEKLY_REVIEW>，并更新顶部闸门进度表；判断按通用安全层引用具体规则。
7. 判灯并按需改计划：黄/红触发或闸门结果需调整时，改 <PLAN_HTML> 逐周课表、计划 vs 实际（canvas id=trk 的实际值数组），版本记录追加 vN 行并递增；比赛成绩更新则同步 runner_profile 与 VDOT。
8. 回复 ≤400 字：上周跑量/完成度、风险颜色、下周改了什么、需要我注意的一件事（含新识别到的传感器/数据断点）。
```
